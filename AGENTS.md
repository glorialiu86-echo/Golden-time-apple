...
- 当前仓库已进入**正式签名 / 上架准备**阶段；`Widget`、`Extension`、`App Group`、真机安装与启动问题都属于正常排查范围，不再按“暂缓迭代”处理。
- 当前真机已验证：`time.golden.GoldenHourCompass` 是当前**可运行且用于正式上架**的标识链路；未获得用户明确指示前，**不要再切换 bundle id / App Group**。
- 卸载并重装宿主 App 会让 iOS / watchOS 桌面上的该 App 小组件被系统一并移除；重装后若发现“小组件没了”，先按系统行为理解，不要误判为 widget 代码被删。
