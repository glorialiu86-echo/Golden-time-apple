...
- 第一阶段只实现 iPhone 宿主 App
- 本轮不实现 watchOS 校对 UI
- 文档中涉及 Watch 的内容仅作为后续规划，不进入当前开发范围
...
- 长按触发应作用于整张 Watch 罗盘页，而不是某个极小热区
- 长按时长定为 `1s`
- 长按成功后，给予一次明确反馈，再进入校对页
- 罗盘页下方的最小字提示始终显示，不做自动出现/消失
...
1. 用户在 Watch 罗盘页长按
2. 进入独立校对页
3. 页面提示用户抬腕并将表盘上方朝向当前太阳方向
4. 用户自行完成对准
5. 用户点击 `保存校对`
6. Watch 本地写入 `watchHeadingOffset`
7. 返回罗盘页，立即生效
...
只有在以下条件同时满足时，Watch 才允许保存校对：

- 已获得可用位置
- 已获得可用 heading
- 当前太阳可见，且 app 已算出太阳方位
- 当前不处于明显剧烈运动状态
...
- 入口提示：`长按进入指南校对`
- 校对页标题：`指南校对`
- 说明：`请抬腕并将表盘上方对准当前太阳方向。`
- 操作说明：`确认对准后，点“保存校对”。`
- 持久化说明：`校对结果会保存在这只 Apple Watch 上。`
- 主按钮：`保存校对`
- 次按钮：`取消`
- 清除按钮：`清除校对`
...
- Entry hint: `Long press to calibrate`
- Title: `Compass Calibration`
- Body: `Raise your wrist and point the top of the watch face toward the sun.`
- Action: `When you're aligned, tap Save Calibration.`
- Persistence: `This calibration is saved on this Apple Watch.`
...
- Watch 校对页必须比 iPhone 版更克制，信息量更少
- 不要在 Watch 上展示大段解释文字
- 不要在 Watch 上做复杂多步引导
- 不要让用户在 Watch 上输入数值或做微调
...
- 手腕姿态变化比手机更大，人工对准误差会更明显
- 表带、表壳、磁性配件更容易持续影响结果
- 走路或摆臂时不适合做校对

因此 Watch 校对应在文案上默认暗示：
